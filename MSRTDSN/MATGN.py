...
# Define MTAGN model
class MTAGNc(nn.Module):
    def __init__(self):
        super(MTAGNc, self).__init__()
        ...
        # Define first layers of every encoder block
        self.encoder_block_1 = nn.ModuleList([
            conv_layer([1, 16], 7),
            conv_layer([16, 32], 5),
            conv_layer([32, 64], 3)
        ])
        ...
        # Define second layers of every encoder block
        self.encoder_block_2 = nn.ModuleList([
            conv_layer([16, 16], 7),
            conv_layer([32, 32], 5),
            conv_layer([64, 64], 3)
        ])

        # Define task attention layers with 3x3 structure
        self.encoder_att = nn.ModuleList([nn.ModuleList([att_layer(3) for _ in range(3)]) for _ in range(3)])

        # Define task conv layers
        self.encoder_att_conv = nn.ModuleList([
            conv_layer([16, 32], 1),
            conv_layer([32, 64], 1),
            conv_layer([64, 128], 1)
        ])
    ...
    def forward(self, x):
        ...
        for i in range(3):
            if i == 0:
                g_encoder.append([
                    self.encoder_block_1[i](x),
                    self.encoder_block_2[i](self.encoder_block_1[i](x))
                ])
                g_maxpool.append(self.maxpool(g_encoder[i][1]))
            else:
                g_encoder.append([
                    self.encoder_block_1[i](g_maxpool[i - 1]),
                    self.encoder_block_2[i](self.encoder_block_1[i](g_maxpool[i - 1]))
                ])
                if i < 2:  # Only apply maxpool to the first two layers
                    g_maxpool.append(self.maxpool(g_encoder[i][1]))

        # Define task dependent module with attention
        atten_encoder = [[[None for _ in range(3)] for _ in range(3)] for _ in range(3)]
        for i in range(3):
            for j in range(3):
                if j == 0:
                    atten_encoder[i][j][0] = self.encoder_att[i][j](g_encoder[j][1])
                    ...
                    atten_encoder[i][j][0] = self.encoder_att[i][j](g_encoder[j][1] + atten_encoder[i][j - 1][2])
                    atten_encoder[i][j][1] = self.encoder_att_conv[j](atten_encoder[i][j][0])
                    atten_encoder[i][j][2] = F.max_pool1d(atten_encoder[i][j][1], kernel_size=(4 if j == 1 else 2),
                                                          stride=(4 if j == 1 else 2))

        # Define task prediction layers
        t1_pred = self.task1_fc(atten_encoder[0][-1][-1])
        t2_pred = self.task2_fc(atten_encoder[1][-1][-1])
        t3_pred = self.task3_fc(atten_encoder[2][-1][-1])
        return t1_pred, t2_pred, t3_pred


# Define MTAGN model
class MTAGNp(nn.Module):
    def __init__(self):
        super(MTAGNp, self).__init__()
        # Initialize network parameters
        filters = [16, 32, 64]
        self.class_1 = 10
        self.class_2 = 4
